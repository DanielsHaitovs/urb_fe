...
export default function PricesCalculator() {
  ...
  const selectedLocation = locationOptions.find((option) => option.id === location) ?? locationOptions[0];
  const selectedExtraDetails = extraServiceOptions.filter((extra) => selectedExtras.includes(extra.id));
  ...
  const breakdown = useMemo(() => {
    ...
    const variableDepth = Math.max(clampedDepth - BASE_PACKAGE.includedDepth, 0) * BASE_PACKAGE.perMeter;
  ...
  const toggleExtraService = (extraId: ExtraServiceId) => {
    setSelectedExtras((current) => (current.includes(extraId) ? current.filter((id) => id !== extraId) : [...current, extraId]));
  };
        ...
        <form className={styles.form}>
          ...
          <div className={styles.addOnsField}>
            ...
            <div className={styles.checkboxGrid}>
              {extraServiceOptions.map((option) => (
                <label key={option.id} className={styles.checkboxCard} data-selected={selectedExtras.includes(option.id)}>
                  <input
                    type="checkbox"
                    value={option.id}
          ...
          <div className={styles.locationField}>
            ...
            <div className={styles.customSelect} ref={dropdownRef}>
              ...
              >
                <div>
                  <strong>{t(`calculator.locations.${selectedLocation.id}`)}</strong>
                  <span>
                ...
                <ul role="listbox" className={styles.dropdownList}>
                  {locationOptions.map((option) => (
                    <li key={option.id}>
                      ...
                      >
                        <div>
                          <strong>{t(`calculator.locations.${option.id}`)}</strong>
                          <span>
                            {option.surcharge === 0
                              ? t("calculator.locationIncluded")
          ...
          <div className={styles.addOnsSummary}>
            {selectedExtraDetails.length > 0 ? (
              selectedExtraDetails.map((extra) => (
                <span key={extra.id}>{t(`calculator.addOns.${extra.id}.label`)}</span>
              ))
            ) : (
