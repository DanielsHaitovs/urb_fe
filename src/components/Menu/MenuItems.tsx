...
export function MenuItems({ screen, isOpen, setOpen }: Props): JSX.Element {
  const t = useTranslations('MenuItems')

  const handleMailClick = (event: MouseEvent<HTMLAnchorElement>): void => {
        ...
        <ul className={styles.items}>
          ...
          {menuItemsList.map((section) => (
            <li key={section.id} className={styles.item}>
              <Link href={`/${section.id}`} onClick={() => setOpen(false)}>
                {t(section.id)}
              </Link>
            </li>
          ))}
